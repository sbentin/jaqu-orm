...
public class Utils {
	...
	public static <T> T newObject(Class<T> clazz) {
		...
		try {
			...
			throw new JaquError("Exception trying to create " + clazz.getName() + ": " + e, e);
		}
	}
	...
	public static <E> E newEnum(Class<E> clazz, int ordinal) {
		...
		throw new JaquError(clazz.getName() + ": is not an enum type"); 
	}
	...
	public static Object convert(Object o, Class<?> targetType) {
		...
		if (targetType == String.class) {
			if (Clob.class.isAssignableFrom(currentType)) {
				...
				try {
					...
					throw new JaquError("Error converting CLOB to String: " + e.toString(), e);
				}
			}
			return o.toString();
		}
		if (null != targetType.getAnnotation(Entity.class) || null != targetType.getAnnotation(MappedSuperclass.class)) {
			...
			try {
				...
				throw new JaquError("Can not convert the value " + o + " from " + currentType + " to " + targetType);
			}
		}
